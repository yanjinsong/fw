			 ...
			 size_t len, int show)
{
	...
	if (((u32)buf >= 192*1024) && (((u32)buf < 0x80000000) || ((u32)buf > (0x80400000-1))))
		{
		LOG_PRINTF("wpa_hexdump invalid address 0x%08X\n", (u32)buf);
		return;
		}
#endif	
	wpa_debug_print_timestamp();
	LOG_PRINTF("%s - hexdump(len=%u):", title, (u32) len);
	if (buf == NULL) {
		LOG_PRINTF(" [NULL]");
	} else if (show) {
	    size_t prt_len = (len > 8U) ? 8U : len;
		for (i = 0; i < prt_len; i++)
			       ...
			       size_t len, int show)
{
	...
	wpa_debug_print_timestamp();
	if (!show) {
		LOG_PRINTF("%s - hexdump_ascii(len=%u): [REMOVED]\n",
		       title, (u32) len);
		return;
	}
	if (buf == NULL) {
		LOG_PRINTF("%s - hexdump_ascii(len=%u): [NULL]\n",
		       title, (u32) len);
		return;
	}
	LOG_PRINTF("%s - hexdump_ascii(len=%u):\n", title, (u32) len);
	while (len) {
		llen = len > line_len ? line_len : len;
		LOG_PRINTF("    ");
		for (i = 0; i < llen; i++)
			LOG_PRINTF(" %02x", pos[i]);
...
#ifdef CONFIG_DEBUG_FILE
	if (out_file) {
		fprintf(out_file, "%s - hexdump(len=%d):",
...
#endif /* CONFIG_DEBUG_FILE */
	printf("%s - hexdump(len=%d):", title, (unsigned long) len);
	if (buf == NULL) {
		printf(" [NULL]");
	} else if (show) {
		for (i = 0; i < len; i++)
			       ...
			       size_t len, int show)
{
	...
	if (out_file) {
		if (!show) {
			fprintf(out_file,
				"%s - hexdump_ascii(len=%d): [REMOVED]\n",
				title, (unsigned long) len);
			return;
		}
		if (buf == NULL) {
			fprintf(out_file,
				"%s - hexdump_ascii(len=%d): [NULL]\n",
				title, (unsigned long) len);
			return;
		}
		fprintf(out_file, "%s - hexdump_ascii(len=%d):\n",
			title, (unsigned long) len);
		while (len) {
			llen = len > line_len ? line_len : len;
			fprintf(out_file, "    ");
...
#endif /* CONFIG_DEBUG_FILE */
	if (!show) {
		printf("%s - hexdump_ascii(len=%d): [REMOVED]\n",
		       title, (unsigned long) len);
		return;
	}
	if (buf == NULL) {
