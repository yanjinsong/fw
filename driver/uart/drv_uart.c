...
s32 drv_uart_init(SSV6XXX_UART_ID_E uart_id)
{
    ...
    else if (uart_id == SSV6XXX_UART1)
    {
        uarts[SSV6XXX_UART1]->SPR = UART1_SPR; // set Baudrate
        //uart->ISR = 1;
        //uart->IER |= 0x1 ; // enable Rx data state output ( also enable UART interrupt )
...
s32 drv_uart_multiple_tx_0 (size_t num, const char *pch)
{
    while (num-- > 0)
        UART0->DATA = (u32)*pch++;
    return 0;
