...
bool bIPDEnable=FALSE;
u32 def_setting_CE01000C=0;
u32 def_setting_CE010008=0;
u32 def_setting_CE010030=0;
u8  ipd_channel_touch=0;
...
static const rfreg_byxtal rfreg_tbl_config_byxtal[] = {
    // _RF_CONFIG_XTAL_26_
    {
        ...
        0xEC4CC5,//0xEC2EC5,   // rg_dpl_rfctrl_f
        1,          // rg_en_dpl_mod
        {
        // {rg_sx_rfctrl_f, rg_sx_rfctrl_ch, rg_sx_target_cnt, rg_sx_refbytwo}
           {      0x89D89E,           0x0B9,           0x0F13,              0},
           {      0xEC4EC5,           0x0B9,           0x0F1B,              0},
           {      0x4EC4EC,           0x0BA,           0x0F23,              0},
           {      0xB13B14,           0x0BA,           0x0F2B,              0},
           {      0x13B13B,           0x0BB,           0x0F33,              0},
           {      0x762762,           0x0BB,           0x0F3B,              0},
           {      0xD89D8A,           0x0BB,           0x0F43,              0},
           {      0x3B13B1,           0x0BC,           0x0F4B,              0},
           {      0x9D89D9,           0x0BC,           0x0F53,              0},
           {      0x000000,           0x0BD,           0x0F5B,              0},
           {      0x627627,           0x0BD,           0x0F63,              0},
           {      0xC4EC4F,           0x0BD,           0x0F6B,              0},
           {      0x276276,           0x0BE,           0x0F73,              0},
           {      0x13B13B,           0x0BF,           0x0F86,              0}
        }
    },
    // _RF_CONFIG_XTAL_40_
    {
...
void drv_phy_tbl_load (const ssv_cabrio_reg tbl[], u32 tbl_size)
{
    ...
    for(loop_i=0; loop_i < tbl_size/sizeof(ssv_cabrio_reg); loop_i++) {
        //Because we can't guartnee the address of tbl is 4 bytes alignment, we copy to the variable "reg"
        memcpy(&_reg,&ptbl[loop_i*sizeof(ssv_cabrio_reg)],sizeof(ssv_cabrio_reg));
        REG32(_reg.address) = _reg.data;
    }
...
void phy_cali_config_load(const phy_cali_config *cfg) {
    ...
    // lock channel
    drv_phy_channel_change(cfg->cfg_ch,FALSE);
...
s32 drv_phy_channel_change(s32 channel_id, bool withRFDisable)
{
//calibration
#define FAIL_MAX 100
u32 cfg_xtal;
...
//                REG32(ADR_LDO_REGISTER)=0x008B7C1C;
        ...
        else
        {
            if(ipd_channel_touch==1){
                ...
                REG32(ADR_ABB_REGISTER_1)=def_setting_CE01000C;
//Mason said no need do this
//                //0xCE010008
//                //SMAC_REG_WRITE(sc->sh, ADR_LDO_REGISTER, sc->def_setting_CE010008);
//                REG32(ADR_LDO_REGISTER)=def_setting_CE010008;
                ////0xCE010030
                //SMAC_REG_WRITE(sc->sh, ADR_RX_ADC_REGISTER, sc->def_setting_CE010030);
                REG32(ADR_RX_ADC_REGISTER)=def_setting_CE010030;
                ipd_channel_touch=0;
            }
        }
    }

    if(withRFDisable== TRUE) SET_RG_MODE(_RF_MODE_STANDBY_);//RF disable
    ...
    do{
        ...
        fail_cnt++;
    }while((fail_cnt < FAIL_MAX) && (phy_sx_lck_bin_rdy != 1));

    if(withRFDisable== TRUE) SET_RG_MODE(_RF_MODE_TRX_EN_); //RF enable
    if(phy_sx_lck_bin_rdy)
    {
        ...
        LOG_PRINTF("Lock of ch %d!!!\n", channel_id);
         ...
         if(channel_id <= 7)
        {
            if(ssv_efuse_config.tx_power_index_1)
            {
                REG32(ADR_RX_TX_FSM_REGISTER)=(REG32(ADR_RX_TX_FSM_REGISTER)&RG_TX_GAIN_OFFSET_I_MSK))|(ssv_efuse_config.tx_power_index_1 << RG_TX_GAIN_OFFSET_SFT);
            ...
            else if(ssv_efuse_config.tx_power_index_2)
            {
                REG32(ADR_RX_TX_FSM_REGISTER)=(REG32(ADR_RX_TX_FSM_REGISTER)&RG_TX_GAIN_OFFSET_I_MSK))
                /*SMAC_REG_READ(sc->sh, ADR_RX_TX_FSM_REGISTER, &regval);
                regval &= RG_TX_GAIN_OFFSET_I_MSK;
                SMAC_REG_WRITE(sc->sh, ADR_RX_TX_FSM_REGISTER, regval);*/
            }
        ...
        {
            if(ssv_efuse_config.tx_power_index_2)
            {
                REG32(ADR_RX_TX_FSM_REGISTER)=(REG32(ADR_RX_TX_FSM_REGISTER)&RG_TX_GAIN_OFFSET_I_MSK))|(ssv_efuse_config.tx_power_index_2 << RG_TX_GAIN_OFFSET_SFT);
                /*SMAC_REG_READ(sc->sh, ADR_RX_TX_FSM_REGISTER, &regval);
                regval &= RG_TX_GAIN_OFFSET_I_MSK;
                regval |= (sh->cfg.tx_power_index_2 << RG_TX_GAIN_OFFSET_SFT);
                SMAC_REG_WRITE(sc->sh, ADR_RX_TX_FSM_REGISTER, regval);*/
            }
            else if(ssv_efuse_config.tx_power_index_1)
            {
                REG32(ADR_RX_TX_FSM_REGISTER)=(REG32(ADR_RX_TX_FSM_REGISTER)&RG_TX_GAIN_OFFSET_I_MSK))
                /*SMAC_REG_READ(sc->sh, ADR_RX_TX_FSM_REGISTER, &regval);
                regval &= RG_TX_GAIN_OFFSET_I_MSK;
...
u8 *pFirmware_phy_setting=NULL;
u8 *pFirmware_rf_setting=NULL;

ssv_rf_temperature firmware_rf_temper_setting[MAX_RF_TEMPER_SETTING_TABLE_SIZE];

u32 firmware_phy_setting_len = 0;
u32 firmware_rf_setting_len = 0;
u32 firmware_rf_temper_setting_len = 0;
...
void drv_phy_reg_init (void) {
    ...
    */
    ...
    if(pFirmware_rf_setting==NULL){
        printf("no rf table \n");
        return;
    }

    if(pFirmware_phy_setting==NULL){
        printf("no phy table \n");
        return;
    }
    ...
    */
    ...
    if(firmware_rf_setting_len){
        drv_phy_tbl_load((ssv_cabrio_reg *)pFirmware_rf_setting, firmware_rf_setting_len);
    }
    ...
    if(firmware_phy_setting_len){
        drv_phy_tbl_load((ssv_cabrio_reg *)pFirmware_phy_setting, firmware_phy_setting_len);
    }
    // ## TX baseband gain (phy related)
...
void drv_phy_evm_tk(u32 enable) {
    ...
    if (enable == 1) {
        ...
        // REG32(0xCE00000C) = 0x07820400; // MCS-7, short-GI
        // REG32(0xCE00000C) = 0x07020400; // MCS-7, normal-GI
        REG32(0xCE00000C) = 0x07010400; // 54M
        REG32(0xCE000020) = 0x20000400;
        REG32(0xCE000004) = 0x0000017F; // turn on PHY
        REG32(0xCE000018) = 0x0055083E; // TX CNT Start
        REG32(0xCE00001C) = 0xFFFFFFFF; // packet number
        REG32(0xCE000018) = 0x0055083D; // Trigger packeten
        REG32(0xCE000018) = 0x0055083F; // PacketGen Enable
    }
    else {
        LOG_PRINTF("# pkt-gen disable\n");

        REG32(0xCE000018) = 0x0055083E; // PacketGen Enable
    }
