...
/* Task: */
OS_APIs s32 OS_TaskCreate( OsTask task, const char *name, u16 stackSize, void *param, u32 pri, OsTaskHandle *taskHandle )
{
    stackSize=stackSize>>2; //The unit of current parameter is bytes.
    return xTaskCreate(
...
OS_APIs s32 OS_MsgQDequeue( OsMsgQ MsgQ, OsMsgQEntry *MsgItem, u32 TickToWait,bool fromISR )
{
    s32 err, val;
	//u32 BlockTick;
    ...
    if (fromISR == false)
        err = xQueueReceive( MsgQ, (void *)MsgItem, TickToWait );
    else{
