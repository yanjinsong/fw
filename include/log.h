...
extern bool		LOG_cli2hcmd(s32 argc, char *argv[], log_hcmd_st	*hcmd);
extern bool		LOG_host_exec_hcmd(log_hcmd_st *hcmd);
#endif
...
#if (CONFIG_SIM_PLATFORM == 1)
#define _log_host_print_status(p)			\
{											\
	_log_prnf_cfg_dump(&((p)->prnf_cfg));	\
	_log_out_desc_dump(&((p)->out_desc));	\
}

extern void		LOG_host_exec_soc_evt(log_soc_evt_st *soc_evt);
#endif
...
extern void sim_printf(u32 n, u32 m, const char *file, u32 line, const char *fmt, ...);
extern void sim_fatal(u32 m, const char *file, u32 line, const char *fmt, ...);

// 'LM' means 'level & module'
#define LOG_PRINTF_LM(l, m, fmt, ...)	(((l) == LOG_LEVEL_FATAL) ? sim_fatal(m, __FILE__, __LINE__, fmt, ##__VA_ARGS__) : sim_printf((l), (m), __FILE__, __LINE__, fmt, ##__VA_ARGS__))
